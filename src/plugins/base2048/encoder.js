...
export const decode = (str) => {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (!(chr in lookupD)) {
      console.error(`Unrecognised Base2048 character: ${chr}`);
    ...
    if (numZBits !== BITS_PER_CHAR && i !== length - 1) {
      console.error(`Secondary character found before end of input at position ${String(i)}`);
